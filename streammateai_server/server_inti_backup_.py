...
class LicenseUpdateUsageRequest(BaseModel):
    email: str = Field(..., min_length=5, max_length=100)
    hours_used: float = Field(..., gt=0, le=24)
...
async def update_user_subscription(
    ...
    try:
        ...
        # PERBAIKAN: Gunakan get_hours_info() bukan get_credit_info() yang tidak ada
        credit_info = license_manager.get_hours_info(email)
        current_balance = credit_info.get("hours_credit", 0)
...
async def check_demo_availability(
    ...
    try:
        ...
        try:
            ...
            credit_info = {"hours_credit": 0}
        
        if credit_info.get("hours_credit", 0) > 50:
            return create_response(
                status="error",
...
async def register_demo(
    ...
    try:
        ...
        current_credit = credit_info.get("hours_credit", 0)
        if current_credit >= 50:
            return create_response(
                status="error",
                message=f"Demo tidak tersedia. Anda masih memiliki {current_credit:.1f} jam kredit. Demo hanya untuk user dengan < 50 jam kredit.",
                status_code=403
            )
...
async def get_credit_balance(
    ...
    try:
        ...
        # PERBAIKAN: Gunakan get_hours_info untuk konsistensi
        credit_info = license_manager.get_hours_info(email)
        ...
        if credit_info:
            return create_response(
                ...
                data={
                    "email": email,
                    # Kembalikan 'hours_credit' untuk kompatibilitas client,
                    # tapi nilainya dari 'hours_credit'
                    "hours_credit": credit_info.get("hours_credit", 0),
                    "credit_balance": credit_info.get("hours_credit", 0),
                    "total_usage": credit_info.get("total_usage", 0),
                    "last_updated": credit_info.get("last_update", datetime.now().isoformat()),
                    "status": "active" if credit_info.get("hours_credit", 0) > 0 else "no_credit"
                }
            )
        else:
            return create_response(
...
async def deduct_credit(
    ...
    try:
        ...
        # PERBAIKAN: Gunakan get_hours_info untuk data terbaru
        current_info = license_manager.get_hours_info(email)
        current_credit = float(current_info.get("hours_credit", 0))
        ...
        # PERBAIKAN: Gunakan 'deduct_credit' yang benar
        ...
        if success:
            ...
            updated_info = license_manager.get_hours_info(email)
            new_credit = float(updated_info.get("hours_credit", 0))
...
async def validate_license(
    ...
    try:
        ...
        license_data = license_manager.get_hours_info(email)
...
async def update_license_usage(
    ...
    """Update kredit usage dari client dengan timeout protection"""
    try:
        email = validate_email(request.email)
        # Nama variabel 'hours_used' kita biarkan agar tidak merusak client,
        # tapi di server kita anggap ini adalah 'credits_used'.
        credits_used = float(request.hours_used)
        ...
        # --- PERBAIKAN UTAMA ---
        # Panggil fungsi DEDUCT_CREDIT dengan timeout protection
        try:
            success = license_manager.deduct_credit(
                email=email,
                amount=credits_used,
            ...
            logger.error(f"[USAGE] Database error for {email}: {db_error}")
            raise HTTPException(status_code=500, detail="Database operation failed")
        
        if success:
            # Dapatkan sisa kredit terbaru dengan timeout protection
            try:
                updated_info = license_manager.get_hours_info(email)
                remaining_credit = updated_info.get("hours_credit", 0)
            except Exception as info_error:
                logger.error(f"[USAGE] Failed to get updated info for {email}: {info_error}")
                remaining_credit = 0
            ...
            return create_response(
                ...
                data={
                    "email": email,
                    "hours_used": credits_used, # Nama field tetap sama untuk client
...
async def admin_add_credit(
    ...
    try:
        ...
        if success:
            # Verifikasi dengan fungsi 'get_hours_info' yang benar.
            verification = license_manager.get_hours_info(email)
            total_credits = verification.get('hours_credit', 0)
